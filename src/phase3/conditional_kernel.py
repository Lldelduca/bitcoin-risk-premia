...
def evaluate_kernel_at_terciles(result, R_grid, Z_matrix, tercile_labels=None, p_phys=None):
    ...
    if tercile_labels is not None:
        ...
        if len(labels) != Z_matrix.shape[0]:
            raise ValueError(
                f"tercile_labels length {len(labels)} != T {Z_matrix.shape[0]}")
        name_iter = ["low", "mid", "high"]
        masks = {name: labels == name for name in name_iter}
    else:
        vol_proxy = Z_matrix[:, 0]
        tercile_edges = np.percentile(vol_proxy, [33.33, 66.67])
        lab_idx = np.digitize(vol_proxy, bins=tercile_edges)
        name_iter = ["low", "mid", "high"]
        masks = {name: lab_idx == i for i, name in enumerate(name_iter)}
